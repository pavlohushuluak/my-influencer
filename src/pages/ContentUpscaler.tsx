...
import { DialogContentZoom, DialogZoom } from "@/components/ui/zoomdialog";
...
const ASPECT_RATIOS = [
  ...
  {
    value: "9:21",
    label: "Ultrawide Portrait 9:21",
    description: "Tall ultrawide",
  },
];
...
export default function ContentUpscaler() {
  ...
  const pollForResult = async (result: UpscaleResult) => {
    const pollInterval = setInterval(async () => {
      try {
        ...
        console.log(
          `${config.supabase_server_url}/generated_images?task_id=eq.${result.taskId}`
        );
        const response = await fetch(
          `${config.supabase_server_url}/generated_images?task_id=eq.${result.taskId}`,
          {
        ...
        if (
          data.length > 0 &&
          data[0].generation_status === "completed" &&
          data[0].system_filename
        ) {
          const completedImage = data[0];
          const upscaledUrl = `${config.data_url}/${userData.id}/${
          ...
          toast.success(
            `${getTaskDisplayName(result.task)} completed successfully`
          );
        } else if (
          data[0].generation_status === "failed" ||
          data[0].generation_status === "error"
        ) {
          clearInterval(pollInterval);
          setIsProcessing(false);
          ...
          setUpscaleResults((prev) =>
            prev.map((r) =>
              r.id === result.id ? { ...r, status: "failed", progress: 0 } : r
            )
          );

          toast.error(
            `${getTaskDisplayName(result.task)} failed: ${
  ...
  const getTaskDisplayName = (task: string) => {
    switch (task) {
      ...
      case "image_upscale_esrgan":
        return "ESRGAN Upscale";
      case "image_realism":
        return "Realism Enhancement";
      case "image_reframe":
        return "Aspect Ratio Reframe";
      default:
        return task;
    }
  };
  ...
  const handleViewFullSize = (result: UpscaleResult) => {
    ...
    const description = `Enhanced using ${taskName} with ${JSON.stringify(
      result.parameters
    )}`;
        ...
        <div className="flex-1 flex flex-col">
          <div className="container mx-auto px-4 py-8 max-w-7xl">
                ...
                <CardContent>
                  <Tabs defaultValue="upscale" className="w-full">
                    ...
                    <TabsContent value="upscale" className="space-y-6 mt-6">
                      ...
                      <Button
                        onClick={() =>
                          processImage("image_upscale", {
                            scaling_factor: parseInt(upscaleScale),
                          })
                        }
                    ...
                    <TabsContent value="esrgan" className="space-y-6 mt-6">
                      ...
                      <Button
                        onClick={() =>
                          processImage("image_upscale_esrgan", {
                            scaling_factor: parseInt(esrganScale),
                          })
                        }
                        disabled={!previewUrl || isProcessing || isCheckingGems}
                    ...
                    <TabsContent value="realism" className="space-y-6 mt-6">
                      ...
                      <Button
                        onClick={() =>
                          processImage("image_realism", {
                            strength: realismStrength[0],
                          })
                        }
                        disabled={!previewUrl || isProcessing || isCheckingGems}
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {upscaleResults.map((result) => (
                      <Card
                        key={result.id}
                        className="overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border-0 bg-white/90 dark:bg-slate-700/90"
                      >
                        <div
                          className="relative cursor-pointer group"
                          onClick={() =>
                            result.status === "completed" &&
                            handleViewFullSize(result)
                          }
                        >
                          <img
                          ...
                          <div className="flex items-center justify-between mb-3">
                            ...
                            >
                              ...
                              <span className="ml-1 font-medium">
                                {getTaskDisplayName(result.task)}
                              </span>
                            </Badge>
            ...
            {showFullSizeModal && fullSizeImage && (
              <DialogZoom
                open={showFullSizeModal}
                onOpenChange={setShowFullSizeModal}
              >
            ...
            <CreditConfirmationModal
              ...
              confirmButtonText={
                gemCostData
                  ? `Confirm & Use ${gemCostData.gems} Credits`
                  : "Confirm"
              }
