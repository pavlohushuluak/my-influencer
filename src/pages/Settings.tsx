...
export default function Settings() {
  ...
  const [monthlyTransactions, setMonthlyTransactions] = useState<
    MonthlyTransaction[]
  >([]);
  const [monthlyTransactionsByItem, setMonthlyTransactionsByItem] = useState<
    MonthlyTransactionByItem[]
  >([]);
  const [
    monthlyTransactionsByTaskTypeUser,
    setMonthlyTransactionsByTaskTypeUser,
  ] = useState<MonthlyTransactionByTaskTypeUser[]>([]);
  const [isLoadingTransactions, setIsLoadingTransactions] = useState(false);
  const [selectedPeriod, setSelectedPeriod] = useState<string>("all");
  const [selectedView, setSelectedView] = useState<"item" | "taskTypeUser">(
    "item"
  );
  ...
  const fetchVaultStats = useCallback(async () => {
    ...
    try {
      ...
      const imagesResponse = await fetch(
        `${config.supabase_server_url}/generated_images?user_uuid=eq.${user.id}&generation_status=eq.completed&select=file_type,file_size_bytes,created_at`,
        {
          headers: {
      ...
      const videosResponse = await fetch(
        `${config.supabase_server_url}/video?user_uuid=eq.${user.id}&status=eq.completed&select=count`,
        {
          headers: {
            Authorization: "Bearer WeInfl3nc3withAI",
            "Content-Type": "application/json",
      ...
      const lastUpdated =
        imagesData.length > 0
          ? new Date(
              Math.max(
                ...imagesData.map((item) => new Date(item.created_at).getTime())
              )
            ).toLocaleDateString()
          : "Never";
  ...
  const fetchMonthlyTransactions = useCallback(async () => {
    ...
    try {
      ...
      const [itemResponse, taskTypeUserResponse] = await Promise.all([
        fetch(
          `${config.supabase_server_url}/v_transactions_monthly_by_item_name_user?user_id=eq.${user.id}`,
          {
            headers: {
              Authorization: "Bearer WeInfl3nc3withAI",
              "Content-Type": "application/json",
        ...
        fetch(
          `${config.supabase_server_url}/v_transactions_monthly_by_task_type_user?user_id=eq.${user.id}`,
          {
            headers: {
              Authorization: "Bearer WeInfl3nc3withAI",
              "Content-Type": "application/json",
  ...
  // Handle print transactions
  const handlePrintTransactions = useCallback(() => {
    const printWindow = window.open("", "_blank");
    if (!printWindow) {
      toast.error("Please allow popups to print transactions");
      return;
    }
    ...
    const totalGems = filteredData.reduce(
      (sum, transaction) => sum + transaction.gems_total,
      0
    );
    const totalUses = filteredData.reduce(
      (sum, transaction) => sum + transaction.uses_count,
      0
    );
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Transaction Log - ${user.email}</title>
          <style>
        ...
        <body>
          ...
          <div class="summary">
            <div class="summary-item">
              <div class="summary-value">${totalGems.toFixed(2)}</div>
              <div class="summary-label">Total Gems Spent</div>
            </div>
            <div class="summary-item">
          ...
          <table>
            <thead>
              <tr>
                <th>Period</th>
                <th>${selectedView === "item" ? "Item Name" : "Task Type"}</th>
                <th>Uses</th>
                <th>Gems Spent</th>
                <th>Avg. per Use</th>
              </tr>
            ...
            <tbody>
              ${filteredData
                .map(
                  (transaction) => `
                <tr>
                  <td class="period">${transaction.period}</td>
                  <td>${
                    "item_name" in transaction
                      ? transaction.item_name
                      : transaction.task_type
                  }</td>
                  <td class="uses">${transaction.uses_count}</td>
                  <td class="gems">${transaction.gems_total.toFixed(2)}</td>
                  <td class="avg">${(
                    transaction.gems_total / transaction.uses_count
                  ).toFixed(2)}</td>
                </tr>
              `
                )
          ...
          <div class="footer">
            <p>Nymia AI - Transaction Report</p>
            <p>This report was generated automatically from your account data.</p>
          </div>
        </body>
      </html>
    `;
  ...
  const getPlanFeatures = (plan: string) => {
    return (
      subscriptionData.features[
        plan.toLowerCase() as keyof typeof subscriptionData.features
      ] || []
    );
  };
  ...
  const getItemIcon = (itemName: string) => {
    ...
    if (item.includes("image") || item.includes("pic"))
      return <Image className="w-4 h-4" />;
    if (item.includes("video")) return <Video className="w-4 h-4" />;
    if (item.includes("lipsync") || item.includes("lip"))
      return <Activity className="w-4 h-4" />;
    if (item.includes("upscale") || item.includes("enhance"))
      return <TrendingUp className="w-4 h-4" />;
    if (item.includes("edit") || item.includes("swap"))
  ...
  const getTaskTypeIcon = (taskType: string) => {
    ...
    if (task.includes("image") || task.includes("pic"))
      return <Image className="w-4 h-4" />;
    if (task.includes("video")) return <Video className="w-4 h-4" />;
    if (task.includes("lipsync") || task.includes("lip"))
      return <Activity className="w-4 h-4" />;
    if (task.includes("upscale") || task.includes("enhance"))
      return <TrendingUp className="w-4 h-4" />;
    if (task.includes("edit") || task.includes("swap"))
      return <BarChart3 className="w-4 h-4" />;
  ...
  const totalGems = filteredTransactions.reduce(
    (sum, transaction) => sum + transaction.gems_total,
    0
  );
  const totalUses = filteredTransactions.reduce(
    (sum, transaction) => sum + transaction.uses_count,
    0
  );
  ...
  const pieChartData = filteredTransactions.reduce((acc, transaction) => {
    ...
    if (existing) {
      ...
      acc.push({
        ...
        color: ("item_name" in transaction
          ? getItemColor(transaction.item_name)
          : getTaskTypeColor(transaction.task_type)
        )
          .replace("text-", "")
          .replace("dark:text-", ""),
      });
  ...
  const monthlyTrendData = filteredTransactions
    .reduce((acc, transaction) => {
      ...
      return acc;
    }, [] as Array<{ month: string; gems: number; uses: number }>)
    ...
    <div className="container mx-auto px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
      ...
      <Tabs defaultValue="profile" className="space-y-4">
        <TabsList className="grid w-full h-full grid-cols-3 sm:grid-cols-3 lg:grid-cols-6 gap-1 p-1">
          <TabsTrigger
            value="profile"
            className="flex flex-col items-center gap-1 py-2 px-1 text-xs sm:flex-row sm:gap-2 sm:text-sm sm:py-2 sm:px-3"
          >
              ...
              <form onSubmit={handleProfileUpdate} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        dispatch(setUser({ firstName: e.target.value }))
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={user.lastName}
                      onChange={(e) =>
                        dispatch(setUser({ lastName: e.target.value }))
                      }
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={user.nickname}
                    onChange={(e) =>
                      dispatch(setUser({ nickname: e.target.value }))
                    }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <Button
                        ...
                        className="bg-white dark:bg-slate-800 border-green-200 dark:border-green-700 text-green-700 dark:text-green-300 hover:bg-green-50 dark:hover:bg-green-950/20"
                      >
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <Button
                        ...
                        className="bg-white dark:bg-slate-800 border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-950/20"
                      >
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription className="text-sm">
                    Complete history of your gem transactions and usage
                  </CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <Button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    By Item Name (Personal)
                  </span>
                  <span className="sm:hidden">Personal Items</span>
                </Button>
                <Button
                  variant={
                    selectedView === "taskTypeUser" ? "default" : "outline"
                  }
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-800">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                          {selectedView === "item" ? "Item Name" : "Task Type"}
                        </th>
                        <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                          Uses
                        </th>
                    ...
                    <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                      {filteredTransactions.map((transaction, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                            {transaction.uses_count}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                            {transaction.gems_total.toFixed(2)}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                            {(
                              transaction.gems_total / transaction.uses_count
