import {
  AgeVerification,
  AgeVerificationData,
} from "@/components/Auth/AgeVerification";
...
export function LegalComplianceManager({
  ...
  const [currentStep, setCurrentStep] = useState<
    "age" | "documents" | "complete"
  >("age");
  const [ageVerificationData, setAgeVerificationData] =
    useState<AgeVerificationData | null>(null);
  const [acceptedDocuments, setAcceptedDocuments] = useState<Set<string>>(
    new Set()
  );
  ...
  const acceptedRequiredDocuments = requiredDocuments.filter((doc) =>
    acceptedDocuments.has(doc.id)
  );
  const progressPercentage = ageVerificationData?.isVerified
    ? ((acceptedRequiredDocuments.length + 1) /
  ...
  const handleComplete = async () => {
    if (
      !ageVerificationData?.isVerified ||
      acceptedRequiredDocuments.length !== requiredDocuments.length
    ) {
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {acceptedRequiredDocuments.length +
                (ageVerificationData?.isVerified ? 1 : 0)}{" "}
              of {requiredDocuments.length + 1} requirements completed
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end pt-4">
                <Button
                  ...
                  disabled={
                    acceptedRequiredDocuments.length !==
                      requiredDocuments.length || isProcessing
                  }
          ...
          {currentStep === "complete" && (
            <div className="text-center space-y-4 py-8">
              ...
              <div>
                ...
                <p className="text-muted-foreground">
                  You have successfully completed all legal requirements and can
