...
const VIDEO_OPTIONS = [
  ...
  },
  {
    ...
    label: "Generate Video Series",
    description: "Generate multiple videos in a series",
  },
];
...
function ContentCreateVideoImage({
  influencerData,
  onBack,
}: ContentCreateVideoImageProps) {
  ...
  const [showSceneSettingsSelector, setShowSceneSettingsSelector] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (influencerId && influencers.length > 0 && !modelData) {
  ...
  }, [searchParams, influencers, modelData]);
  ...
  useEffect(() => {
    const fetchVideoModels = async () => {
      try {
        ...
        const response = await fetch(
          `${config.backend_url}/fieldoptions?fieldtype=video_model`,
          {
            headers: {
              Authorization: "Bearer WeInfl3nc3withAI",
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchVideoLengths = async () => {
      try {
        ...
        const response = await fetch(
          `${config.backend_url}/fieldoptions?fieldtype=video_length`,
          {
            headers: {
              Authorization: "Bearer WeInfl3nc3withAI",
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchVideoResolutions = async () => {
      try {
        ...
        const response = await fetch(
          `${config.backend_url}/fieldoptions?fieldtype=video_resolution`,
          {
            headers: {
              Authorization: "Bearer WeInfl3nc3withAI",
              "Content-Type": "application/json",
  ...
  useEffect(() => {
    if (influencerData) {
      ...
      setModelDescription({
        appearance: `${influencerData.name_first} ${
          influencerData.name_last
        ...
        makeup: "Natural / No-Makeup Look",
        clothing: `${influencerData.clothing_style_everyday || ""} ${
        ...
        colorPalette: influencerData.color_palette
          ? influencerData.color_palette.join(", ")
          : "",
      ...
      toast.success(
        `Using ${influencerData.name_first} ${influencerData.name_last} for video generation`
      );
    }
  }, [influencerData]);
    ...
    setFormData((prev) => {
      ...
      if (field === "engine") {
        ...
        if (!allowedResolutions.includes(newData.format)) {
          ...
          toast.info(
            `Resolution automatically adjusted to ${allowedResolutions[0]} for ${value}`
          );
        }
        ...
        if (value.includes("kling")) {
          newData.mode =
            newData.format === "480p" || newData.format === "720p"
              ? "standard"
              : "pro";
        } else {
          newData.mode = "";
  ...
  const getVideoItemName = (
    ...
    if (engine === "kling-v2.1") {
      return durationNum === 5
        ? "kling_video_standard_5s"
        : "kling_video_standard_10s";
    }
    if (engine === "kling-v2.1-master") {
      return durationNum === 5
        ? "kling_video_master_5s"
        : "kling_video_master_10s";
    }
    ...
    if (engine === "seedance-1-lite") {
      if (format === "480p") {
        return durationNum === 5
          ? "seedance_lite_480p_5s"
          : "seedance_lite_480p_10s";
      } else if (format === "720p") {
        return durationNum === 5
          ? "seedance_lite_720p_5s"
          : "seedance_lite_720p_10s";
      }
    }
    if (engine === "seedance-1-pro") {
      return durationNum === 5
        ? "seedance_pro_1080p_5s"
        : "seedance_pro_1080p_10s";
    }
  ...
  const executeVideoGeneration = async () => {
    ...
    try {
      ...
      if (formData.engine.includes("kling")) {
        ...
        const mode =
          formData.format === "480p" || formData.format === "720p"
            ? "standard"
            : "pro";
        videoGenerationData = {
          ...
          start_image: modelData?.image_url
            ? modelData.image_url.split("/").pop() || ""
            : "",
      ...
      if (response.ok) {
        ...
        toast.success(
          "Video generation started! Check your history for progress."
        );
  ...
  const handleGenerate = async () => {
    ...
    const gemData = await checkVideoGemCost(
      formData.engine,
      formData.duration,
      formData.format
    );
  ...
  const handleSavePreset = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${config.supabase_server_url}/video_presets`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleApplyVideoPreset = (preset: any) => {
    ...
    if (finalResolution !== presetResolution) {
      toast.info(
        `Resolution automatically adjusted to ${finalResolution} for ${engine}`
      );
    }
  ...
  const handleModelSelect = (model: VideoModel) => {
    setFormData((prev) => {
      ...
      if (!allowedResolutions.includes(newData.format)) {
        newData.format = allowedResolutions[0];
        toast.info(
          `Resolution automatically adjusted to ${allowedResolutions[0]} for ${model.label}`
        );
      }
  ...
  const handleResolutionSelect = (resolution: VideoResolution) => {
    setFormData((prev) => {
      ...
      if (prev.engine.includes("kling")) {
        newData.mode =
          resolution.label === "480p" || resolution.label === "720p"
            ? "standard"
            : "pro";
      }
    ...
    <div className="min-h-screen">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm sm:text-base">
              {modelData
                ? `Creating content for ${modelData.name_first} ${modelData.name_last}`
      ...
      <div className="flex h-[calc(100vh-80px)]">
        ...
        <div className="flex-1 px-6 space-y-4">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant="outline"
                        className="text-xs text-orange-600 border-orange-200 dark:text-orange-400 dark:border-orange-700"
                      >
                        Optional
                      </Badge>
                    </div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Note: Negative prompts are not supported by all video
                      models. Leave empty if your model doesn't support them.
                    </p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              <>
                                ...
                                <div className="text-left">
                                  <p className="font-medium text-slate-900 dark:text-slate-100">
                                    {videoModels.find(
                                      (m) => m.label === formData.engine
                                    )?.label || formData.engine}
                                  </p>
                          ...
                          <div className="flex items-center gap-3">
                            {formData.format ? (
                              <>
                                ...
                                <div className="text-left">
                                  <p className="font-medium text-slate-900 dark:text-slate-100">
                                    {videoResolutions.find(
                                      (r) => r.label === formData.format
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        value={formData.seed}
                        onChange={(e) =>
                          handleInputChange("seed", e.target.value)
                        }
                        className="w-full"
                      />
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Enter a number to control randomness. Leave empty for
                        random seed.
                      </p>
                    </div>
                  </div>
            ...
            <Dialog open={showHistory} onOpenChange={setShowHistory}>
              <DialogContent className="max-w-6xl max-h-[80vh]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Your video generation history. Showing{" "}
                ...
                <div className="flex justify-between items-center pt-4 border-t">
                  <div className="text-sm text-muted-foreground">
                    ...
                    of {regularVideos.length} videos
                  </div>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Save your current video generation settings as a reusable
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <Card
                          ...
                          onClick={() => {
                            ...
                            document
                              .getElementById("file-upload-direct")
                              ?.click();
                          }}
                        >
                          <CardContent className="p-6 text-center">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm mt-1">
                            {selectedVideoForModal.mode || "Standard"}
                          </p>
                        </div>
                        <div>
                          <Label className="text-sm font-medium text-muted-foreground">
                        ...
                        <div>
                          ...
                          <p className="text-sm mt-1">
                            {selectedVideoForModal.resolution || "HD"}
                          </p>
                        </div>
                      </div>
                    </div>
          ...
          <Dialog open={showModelSelector} onOpenChange={setShowModelSelector}>
            <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription>
                  Choose the best video generation model for your content. Each
                  model has different capabilities and quality levels.
                </DialogDescription>
              </DialogHeader>
                              ...
                              <img
                                ...
                                onError={(e) => {
                                  ...
                                  e.currentTarget.parentElement!.className =
                                    "aspect-video bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center";
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-1 pt-2">
                              {model.label.includes("pro") && (
                                <Badge
                                  variant="outline"
                                  className="text-xs border-purple-200 dark:border-purple-700 text-purple-700 dark:text-purple-400"
                                >
                                  Pro Version
                                </Badge>
                              )}
                              ...
                              {model.label.includes("1080p") && (
                                <Badge
                                  variant="outline"
                                  className="text-xs border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-400"
                                >
                                  1080p
                                </Badge>
                              )}
          ...
          <Dialog
            open={showLengthSelector}
            onOpenChange={setShowLengthSelector}
          >
            <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                ...
                <DialogDescription>
                  Choose the perfect duration for your video content. Different
                  lengths work better for different platforms and purposes.
                </DialogDescription>
              </DialogHeader>
                            ...
                            <div className="aspect-video bg-gradient-to-br from-green-100 to-emerald-100 dark:from-green-900/30 dark:to-emerald-900/30 rounded-xl overflow-hidden shadow-md">
                              <img
                                ...
                                onError={(e) => {
                                  ...
                                  e.currentTarget.style.display = "none";
                                  e.currentTarget.parentElement!.className =
                                    "aspect-video bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center";
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex flex-wrap gap-1 pt-2">
                              ...
                              {parseInt(length.label) >= 30 && (
                                <Badge
                                  variant="outline"
                                  className="text-xs border-indigo-200 dark:border-indigo-700 text-indigo-700 dark:text-indigo-400"
                                >
                                  YouTube
                                </Badge>
                              )}
          ...
          <Dialog
            ...
            onOpenChange={setShowResolutionSelector}
          >
            <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] overflow-hidden">
              <DialogHeader className="pb-6">
                ...
                <DialogDescription className="text-base text-slate-600 dark:text-slate-400">
                  Choose the perfect resolution for your video content. Each
                  option is optimized for specific platforms and use cases.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="flex-1 pr-2">
                {loadingResolutions ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {videoResolutions
                      .filter((resolution) =>
                        getAllowedResolutions(formData.engine).includes(
                          resolution.label
                        )
                      )
                      .map((resolution) => (
                              ...
                              <div className="h-48 bg-gradient-to-br from-purple-100 via-pink-100 to-indigo-100 dark:from-purple-900/30 dark:via-pink-900/30 dark:to-indigo-900/30 overflow-hidden">
                                <img
                                  ...
                                  onError={(e) => {
                                    e.currentTarget.style.display = "none";
                                    e.currentTarget.parentElement!.className =
                                      "h-48 bg-gradient-to-br from-purple-500 via-pink-500 to-indigo-500 flex items-center justify-center";
                                ...
                                <div className="flex flex-wrap gap-2">
                                  {resolution.label === "720p" && (
                                    <>
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20"
                                      >
                                        <svg
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-green-200 dark:border-green-700 text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-purple-200 dark:border-purple-700 text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
                                  ...
                                  {resolution.label === "1080p" && (
                                    <>
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-indigo-200 dark:border-indigo-700 text-indigo-700 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-pink-200 dark:border-pink-700 text-pink-700 dark:text-pink-400 bg-pink-50 dark:bg-pink-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
                                      ...
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-orange-200 dark:border-orange-700 text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
                                  ...
                                  {resolution.label === "480p" && (
                                    <>
                                      <Badge
                                        variant="outline"
                                        className="text-xs border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20"
                                      >
                                        <svg
                                          className="w-3 h-3 mr-1"
                                          fill="currentColor"
              ...
              <div className="flex justify-between items-center pt-6 border-t border-slate-200 dark:border-slate-700">
                <div className="text-sm text-slate-500 dark:text-slate-400">
                  {
                    videoResolutions.filter((resolution) =>
                      getAllowedResolutions(formData.engine).includes(
                        resolution.label
                      )
                    ).length
                  }{" "}
                  resolution options available for {formData.engine}
                </div>
