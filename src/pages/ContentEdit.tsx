...
export default function ContentEdit() {
  ...
  const [editMode, setEditMode] = useState<
    "selection" | "image-edit" | "ai-image-edit" | "ai-synthesis"
  >("selection");
  ...
  const [synthesisImages, setSynthesisImages] = useState<
    Array<{
      id: string;
      name: string;
      url: string;
  ...
  >([]);
  const [showSynthesisImageSelector, setShowSynthesisImageSelector] =
    useState(false);
  ...
  const influencersLoading = useSelector(
  ...
  const handleAiImageSelect = (image: any) => {
    try {
      ...
      const isInfluencerImage =
        imageData.file_path?.includes("models/") ||
        imageData.id?.startsWith("influencer-");
  ...
  const addSynthesisImage = (imageData: {
    id: string;
    name: string;
    url: string;
    source: "library" | "influencer" | "upload";
  ...
  const handleSynthesisImageSelect = (imageData: {
    url: string;
    source: "library" | "influencer" | "upload";
    tempId?: string;
  }) => {
  ...
  const processSynthesis = async () => {
    ...
    try {
      ...
      const referenceTags =
        synthesisPrompt.match(/@(\w+)/g)?.map((tag) => tag.slice(1)) || [];
  ...
  const processAiEdit = async () => {
    ...
    try {
      ...
      const maskVaultUrl = await uploadImageToVault(
        maskBlob,
        "mask",
        "ai_edit"
      );
      ...
      const response = await fetch(
        `${config.backend_url}/createtask?userid=${useridData[0].userid}&type=ai_edit_image`,
        {
          method: "POST",
          headers: headers,
          body: JSON.stringify(payload),
  ...
  const handleResizeMove = useCallback(
    (e: MouseEvent) => {
      ...
      if (
        e.target &&
        (e.target as Element).closest("[data-resize-direction]")
      ) {
        const direction = (e.target as Element)
          .closest("[data-resize-direction]")
          ?.getAttribute("data-resize-direction");
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MouseEvent) => {
      if (
        sizeControlsRef.current &&
        !sizeControlsRef.current.contains(e.target as Node)
      ) {
  ...
  const fetchImage = async (imageData: ImageData) => {
    ...
    try {
      ...
      const isInfluencerImage =
        imageData.file_path?.includes("models/") ||
        imageData.id?.startsWith("influencer-");
      const filename = isInfluencerImage
        ? imageData.file_path
        : "output/" + imageData.system_filename;
  ...
  useEffect(() => {
    ...
    if (
      imageData &&
      userData?.id &&
      (!selectedImage || selectedImage.id !== imageData.id)
    ) {
      setSelectedImage(imageData);
      fetchImage(imageData);
    }
  ...
  const handleDownloadEdited = useCallback(async () => {
    ...
    const blob = await response.blob();
    const file = new File([blob], selectedImage.system_filename, {
      type: "image/jpeg",
    });
      ...
      try {
        ...
        if (getFilesResponse.ok) {
          ...
          if (files && files.length > 0 && files[0].Key) {
            ...
            const baseName = filenameToUse.substring(
              0,
              filenameToUse.lastIndexOf(".")
            );
            const extension = filenameToUse.substring(
              filenameToUse.lastIndexOf(".")
            );
        ...
        // Upload file to API
        const uploadResponse = await fetch(
          `${config.backend_url}/uploadfile?user=${userData.id}&filename=output/${finalFilename}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/octet-stream",
        ...
        const dbResponse = await fetch(
          `${config.supabase_server_url}/generated_images`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        if (!dbResponse.ok) {
          ...
          throw new Error(
            `Failed to create database entry: ${dbResponse.status} ${errorText}`
          );
        }

        toast.dismiss(loadingToast);
        toast.success(
          `Image uploaded to library successfully as "${decodeFilename(
            finalFilename
          )}"!`
        );
      } catch (error) {
  ...
  const handleOverwriteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const dbDeleteResponse = await fetch(
        `${config.supabase_server_url}/generated_images?system_filename=eq.${conflictFilename}&user_filename=eq.`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!dbDeleteResponse.ok) {
        console.warn(
          "Failed to delete old database entry, but continuing with upload"
        );
      }
      ...
      // Create a file from the blob
      const file = new File([pendingUploadData.blob], conflictFilename, {
        type: "image/jpeg",
      });

      // Upload new file
      const uploadResponse = await fetch(
        `${config.backend_url}/uploadfile?user=${userData.id}&filename=output/${conflictFilename}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/octet-stream",
      ...
      if (!dbResponse.ok) {
        const errorText = await dbResponse.text();
        console.error(
          "Database response error (overwrite):",
          dbResponse.status,
          errorText
        );
      ...
      toast.dismiss(loadingToast);
      toast.success(
        `File "${decodeFilename(conflictFilename)}" overwritten successfully!`
      );
    } catch (error) {
    ...
    try {
      ...
      if (getFilesResponse.ok) {
        ...
        if (files && files.length > 0 && files[0].Key) {
          ...
          const baseName = selectedImage.system_filename.substring(
            0,
            selectedImage.system_filename.lastIndexOf(".")
          );
          const extension = selectedImage.system_filename.substring(
            selectedImage.system_filename.lastIndexOf(".")
          );
      ...
      // Create a file from the blob with the unique filename
      const file = new File([pendingUploadData.blob], finalFilename, {
        type: "image/jpeg",
      });
      ...
      if (!dbResponse.ok) {
        const errorText = await dbResponse.text();
        console.error(
          "Database response error (create new):",
          dbResponse.status,
          errorText
        );
      ...
      toast.dismiss(loadingToast);
      toast.success(
        `New file created successfully as "${decodeFilename(finalFilename)}"!`
      );
    } catch (error) {
          ...
          reader.onload = async (e) => {
            ...
            if (editMode === "ai-image-edit") {
              ...
              try {
                ...
                toast.dismiss(loadingToast);
                toast.error(
                  "Failed to upload image to vault. Please try again."
                );
                return;
              }
            } else if (editMode === "ai-synthesis") {
  ...
  console.log(
    "ContentEdit: filteredInfluencers count",
  ...
  const handleInfluencerSelect = (influencer: Influencer) => {
    try {
      ...
      if (editMode === "ai-image-edit") {
        ...
        toast.success(
          `Selected ${influencer.name_first} ${influencer.name_last} for AI editing`
        );
      } else {
        ...
        toast.success(
          `Selected ${influencer.name_first} ${influencer.name_last} for editing`
        );
      }
    } catch (error) {
  ...
  if (!selectedImage && showImageSelection) {
    return (
      <div>
        <div className="pb-6 md:pb-8 space-y-8">
          {/* Enhanced Header */}
          ...
          <Dialog
            open={showInfluencerSelector}
            onOpenChange={setShowInfluencerSelector}
          >
                ...
                {influencersLoading ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredInfluencers.map((influencer) => (
                      <Card
                        key={influencer.id}
                        className="group hover:shadow-lg transition-all duration-300 border-border/50 hover:border-purple-500/20"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
            ...
            {/* Upload New Image */}
            <Card
              className="group hover:shadow-2xl transition-all duration-500 cursor-pointer border-0 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm"
        ...
        <Card>
          ...
          <CardContent>
            ...
            >
              ...
              <p className="text-sm md:text-base text-gray-500 mb-4 text-center px-4">
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 sm:p-6 border-b border-border/50 bg-gradient-to-r from-purple-50/50 to-blue-50/50 dark:from-purple-950/20 dark:to-blue-950/20 space-y-4 sm:space-y-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-sm sm:text-base">
                Advanced image editing with professional tools
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center gap-6 relative">
                ...
                <div className="flex items-center gap-1 bg-white/80 dark:bg-gray-900/80 rounded-full shadow px-2 py-1 border border-blue-100 mr-2">
                  {THEME_MODES.map((opt) => (
                    ...
                    ${
                      ...
                      style={{
                        outline:
                          themeMode === opt.key ? "2px solid #2563eb" : "none",
                      }}
                      title={opt.label}
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  {showSizeControls && (
                    <div
                      ref={sizeControlsRef}
                      className="absolute top-full left-0 mt-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4 min-w-[280px] z-50"
                    >
                      <div className="space-y-4">
                            ...
                            {PRESET_SIZES.map((preset) => (
                              <button
                                ...
                                className={`px-3 py-2 text-xs rounded border transition-all ${
                                  editorWidth === preset.width &&
                                  editorHeight === preset.height
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            Enter values between 400-2000px for width and
                            300-1500px for height
                          </div>
                        </div>
                ...
                {showCustomModal && (
                  <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/30">
                    <div
                      ref={modalRef}
                      className="bg-white dark:bg-gray-900 border border-blue-200 rounded-2xl shadow-2xl px-8 py-6 flex flex-col items-center relative animate-fade-in"
                    >
                      <button
                      ...
                      <div className="mb-3 text-base text-gray-700 dark:text-gray-200 font-semibold">
                        Custom Color Theme
                      </div>
                      ...
                      <div className="text-xs text-gray-500">
                        Toggle colors to create your custom background
                      </div>
                    </div>
                  </div>
                )}
            ...
            <CardContent className="p-6">
              ...
              {!hasImage ? (
                ...
                >
                  ...
                  <h3 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3 text-center">
                    Upload an image to edit
                  </h3>
                  <p className="text-base md:text-lg text-slate-600 dark:text-slate-300 mb-6 text-center px-8 leading-relaxed">
                ...
                <div
                  className="border-2 border-slate-200 dark:border-slate-700 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 flex flex-col items-center justify-center"
                ...
                >
                  ...
                  <p className="text-base text-slate-600 dark:text-slate-300 text-center px-8">
                    Loading and optimizing your image for professional editing
                  </p>
                </div>
              ) : (
                <div
                  className={`border-2 border-slate-200 dark:border-slate-700 rounded-2xl bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-900 relative transition-all duration-300 ${
                    isResizing
        ...
        {showVaultSelector && (
          <VaultSelector
            ...
            onOpenChange={setShowVaultSelector}
            onImageSelect={
              editMode === "ai-image-edit"
                ? handleAiImageSelect
                : handleVaultImageSelect
            }
            title={
              editMode === "ai-image-edit"
                ? "Select Image for AI Editing"
                : "Select Image from Library"
            }
            description={
              editMode === "ai-image-edit"
                ? "Browse your library and select an image to edit with AI. Only completed images are shown."
                : "Browse your library and select an image to edit. Only completed images are shown."
            }
          />
        )}
        ...
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                A file named "{conflictFilename}" already exists in the library.
            ...
            <div className="space-y-4">
              <div className="p-4 bg-yellow-50 dark:bg-yellow-950/20 rounded-lg">
                <p className="text-sm text-yellow-800 dark:text-yellow-200">
                  <strong>Warning:</strong> Overwriting will permanently replace
                  the existing file.
                </p>
              </div>
        ...
        <Dialog open={showFilenameDialog} onOpenChange={setShowFilenameDialog}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                You can use your own filename for this image. Please edit below
                if you wish.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
